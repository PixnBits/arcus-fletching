...
var uPnpRootDevice = require('./uPnpRootDevice');
...
uPnpControlPoint.prototype._analyzeBroadcastMessage = function(message, sender){
	...
	switch(msg.method.toLowerCase()){
		case 'notify':
			...
			}else if('ssdp:byebye' === msg.headers.nts){
				this._removeDeviceByAdvertisement(msg);
			}else{
...
uPnpControlPoint.prototype._removeDeviceByAdvertisement = function(advertisement){
	...
	if(usn.uuid){
		// maybe in both lists? :S
		var device = this.rootDevices[usn.uuid] || this.devices[usn.uuid];
		this.rootDevices[usn.uuid] = this.devices[usn.uuid] = null;
		if(!device){
			// wasn't ever added?
			console.warn('device left before being discovered?', usn.uuid);
		}else{
			this.emit('left', device);
		}
	}else{
		console.error('unable to find device uuid?!?', usn, advertisement);
	}
